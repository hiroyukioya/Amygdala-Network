...
t=linspace(-1,1,16000);
 for ii= Used_ROI;
      Count=Count+1;
       ...
       %  subplot(6,7,Count)
         ...
         else [h p] =   ttest2(Features(Medial_index,:), Features(Lateral_index,:));
         end
 ...
 Multiple_comparison=numel(Ps(:,1))- numel( find(isnan(Ps(:,1))==1)  )
...
Pslog=-log10(Ps);
Lateralsmall_index=find(Subtractions(:,1) > 0);    %%%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!here! change samll of large !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Pslog(Lateralsmall_index,1)=NaN; 

Max_late=Pslog(:,1);      %%%%%%%%%%%%%%%%%%%%%%here to change the target period !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Max_late(abs(Max_late) < -log10(Corrected_alpha)) =NaN;
Mediallarge_index=find(Subtractions( :,1) > 0);
